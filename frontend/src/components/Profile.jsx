    ...
    <div className="min-h-screen bg-gray-100 pb-5">
      ...
      <div className='flex flex-row items-center justify-center gap-6 flex-wrap'>
        <div className="flex flex-col items-center mt-2 px-4">
          <div className="rounded-full border-4 border-indigo-200 h-36 w-36 flex items-center justify-center bg-indigo-100 shadow-lg mt-3">
            <span className="text-5xl font-bold text-indigo-600">
              {user?.username?.charAt(0)?.toUpperCase() || "U"}
            </span>
          </div>
          <p className="mt-3 text-4xl font-bold text-gray-900">{user?.username || "Guest"}</p>
          <p className="text-lg text-gray-600 mt-1">{user?.email || "No email"}</p>
          {user?.fullname && (
